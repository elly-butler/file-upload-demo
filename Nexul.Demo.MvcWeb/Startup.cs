...
using Nexul.Demo.MvcWeb.Auth;
using Nexul.Demo.MvcWeb.Filters;
using Nexul.Demo.SqlDbServices;
using Nexul.Demo.Files;

namespace Nexul.Demo.MvcWeb
{
    public class Startup
    {
        private const string SecretKey = "12345678901234567890123456789012"; //TODO: make up a secure key and get from somewhere secure
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddDbContext<NexulDemoDbContext>(options =>
                options.UseSqlServer(Configuration.GetConnectionString("DefaultConnection")));

            var dbAssemblyName = typeof(NexulIdentityDbContext).Namespace;
            services.AddDbContext<NexulIdentityDbContext>(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("DefaultConnection"),
                    sqlOptions => sqlOptions.MigrationsAssembly(dbAssemblyName)
                ));

            var identityBuilder = services.AddIdentityCore<NexulIdentityUser>(o =>
            ...
            identityBuilder.AddEntityFrameworkStores<NexulIdentityDbContext>();
            identityBuilder.AddRoleValidator<RoleValidator<IdentityRole>>();
            identityBuilder.AddRoleManager<RoleManager<IdentityRole>>();
            identityBuilder.AddSignInManager<SignInManager<NexulIdentityUser>>();
